import streamlit as st
from database import verificar_usuario, conectar_bd 

def registrar_acceso_auditoria(usuario, nombre, rol):
    """
    Inserta un registro en la tabla log_accesos al iniciar sesión con éxito.
    Sincronizado con el esquema de simpol.sql.
    """
    try:
        conn = conectar_bd()
        if conn:
            cursor = conn.cursor()
            # La tabla log_accesos espera: usuario, nombre_completo, rol, ip_cliente, resultado
            query = """
                INSERT INTO log_accesos (usuario, nombre_completo, rol, ip_cliente, resultado) 
                VALUES (%s, %s, %s, '127.0.0.1', 'EXITOSO')
            """
            cursor.execute(query, (usuario, nombre, rol))
            conn.commit()
            cursor.close()
            conn.close()
    except Exception as e:
        # Se usa print para no interrumpir el flujo visual del login si falla la auditoría
        print(f"Error en auditoría de acceso: {e}")

def mostrar_login():
    """
    Renderiza la interfaz de acceso con la estética del Banco Caroní.
    """
    # Centrado visual del formulario
    _, col2, _ = st.columns([1, 2, 1])

    with col2:
        st.markdown('<div class="login-container">', unsafe_allow_html=True)
        st.markdown(
            "<h1 style='text-align: center; color: #003366;'>SIMPOL</h1>",
            unsafe_allow_html=True,
        )
        st.markdown(
            "<h4 style='text-align: center; color: #333333;'>Sistema Inteligente de Monitoreo Permanente Online</h4>",
            unsafe_allow_html=True,
        )
        st.write("---")

        with st.form("login_form"):
            st.markdown(
                "<p style='color: #003366; font-weight: bold;'>🔐 Acceso al Sistema</p>",
                unsafe_allow_html=True,
            )
            usuario = st.text_input("Usuario")
            clave = st.text_input("Contraseña", type="password")

            # Botón de ingreso con estilo institucional configurado en style.css
            if st.form_submit_button("INGRESAR", use_container_width=True):
                # verificar_usuario devuelve un diccionario con: id, usuario, nombre_completo, rol
                user_data = verificar_usuario(usuario, clave)
                
                if user_data:
                    # 1. PERSISTENCIA DE SESIÓN (Punto crítico para llaves foráneas)
                    st.session_state["autenticado"] = True
                    st.session_state["user_id"] = user_data["id"]  # ID numérico de la BD
                    st.session_state["user_actual"] = user_data["usuario"]
                    st.session_state["nombre_analista"] = user_data["nombre_completo"]
                    st.session_state["rol"] = user_data["rol"].lower()
                    
                    # 2. REGISTRO DE AUDITORÍA
                    registrar_acceso_auditoria(
                        user_data["usuario"], 
                        user_data["nombre_completo"], 
                        user_data["rol"]
                    )
                    
                    # 3. ACTUALIZACIÓN DE PARÁMETROS URL (Para restaurar sesión si se refresca)
                    st.query_params.update({
                        "session": "active",
                        "uid": user_data["id"],
                        "u": user_data["usuario"],
                        "n": user_data["nombre_completo"],
                        "r": user_data["rol"]
                    })
                    
                    # Refrescar para entrar al orquestador (app.py)
                    st.rerun()
                else:
                    st.error("Credenciales incorrectas o cuenta suspendida. Contacte al Oficial de Seguridad.")
        
        st.markdown('</div>', unsafe_allow_html=True)

    # Footer institucional opcional
    st.markdown(
        """
        <div style='text-align: center; color: #999; font-size: 12px; margin-top: 50px;'>
            © 2026 SIMPOL - Departamento de Infraestructura y Redes
        </div>
        """, 
        unsafe_allow_html=True
    )