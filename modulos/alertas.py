...
def obtener_umbrales_actuales(ip):
    # Diccionario base adaptado fielmente a las columnas reales de 'historico_umbrales'
    umbrales = {
        "cpu_buen_estado": 69.0, "cpu_advertencia": 70.0, "cpu_critico": 85.0,
        "cpu_p_buen_estado": 69.0, "cpu_p_advertencia": 70.0, "cpu_p_critico": 85.0,
        "ram_buen_estado": 20.0, "ram_advertencia": 15.0, "ram_critico": 10.0,
        "red_limite_total_mbps": 100.0, "red_limite_entrante_mbps": 50.0, "red_limite_saliente_mbps": 50.0,
        "latencia_limite_ms": 150.0, "perdida_limite_pct": 1.0
    }

    # Umbrales estáticos para los 6 discos en la BD
    for i in range(1, 7):
...
def guardar_nuevos_umbrales(ip, dict_umbrales, usuario_id, justificacion):
    ...
    try:
        ...
        # Mapeo de los 6 discos fijos de la base de datos
        for i in range(1, 7):
        ...
        # Columnas de red segmentadas y calidad de canal según el CREATE TABLE
        columnas.extend([
...
def renderizar_bloque_red_latencia(titulo, valor, limite, unidad, es_perdida=False):
    if es_perdida:
        # Corregido: Ahora evalúa de forma dinámica contra el umbral guardado en la BD
        if valor >= (limite * 5.0):
...
def renderizar_estado_servicio(nombre_servicio, estado_enum, id_sensor):
    st.markdown(f"**{nombre_servicio}** (Sensor #{id_sensor}): {estado_enum}")

# =====================================================================
# VISTA PRINCIPAL (MÓDULO DE ALERTAS)
# =====================================================================
...
def mostrar_pantalla(nombre_analista="Analista", usuario_id=1, usuario_login="Sistema"):
                        ...
                        if not telemetria:
                            ...
                            if tiene_cpu:
                                ...
                                for c_idx in range(1, 9):
                                    ...
                                    with col_destino:
                                        # Corregido: Usa la columna genérica 'cpu_p_' definida en la estructura SQL
                                        renderizar_barra_cpu(
                                            titulo=f"Core {c_idx}",
                                            pct_uso=val_core,
                                            umbral_adv=umbrales.get("cpu_p_advertencia", 70.0),
                                            umbral_crit=umbrales.get("cpu_p_critico", 85.0),
                            ...
                            with col_red_izq:
                                ...
                                renderizar_bloque_red_latencia("Tráfico Total", float(telemetria.get('val_red_total') or 0.0), umbrales.get("red_limite_total_mbps", 100.0), "Mbps")
                                renderizar_bloque_red_latencia("Tráfico Entrante", float(telemetria.get('val_red_entrante') or 0.0), umbrales.get("red_limite_entrante_mbps", 50.0), "Mbps")
                                renderizar_bloque_red_latencia("Tráfico Saliente", float(telemetria.get('val_red_saliente') or 0.0), umbrales.get("red_limite_saliente_mbps", 50.0), "Mbps")
                            ...
                            with col_red_der:
                                ...
                                renderizar_bloque_red_latencia("Ping Promedio", float(telemetria.get('val_latencia_ping') or 0.0), umbrales.get("latencia_limite_ms", 150.0), "ms")
                                renderizar_bloque_red_latencia("Latencia Máxima", float(telemetria.get('val_latencia_max') or 0.0), umbrales.get("latencia_limite_ms", 150.0), "ms")
                                renderizar_bloque_red_latencia("Latencia Mínima", float(telemetria.get('val_latencia_min') or 0.0), umbrales.get("latencia_limite_ms", 150.0), "ms")
                                # Corregido: 'es_loss' cambiado por 'es_perdida' para evitar crash de Python
                                renderizar_bloque_red_latencia("Pérdida de Paquetes", float(telemetria.get('val_latencia_perdida') or 0.0), umbrales.get("perdida_limite_pct", 1.0), "%", es_perdida=True)
                                ...
                                for idx, disco in enumerate(discos_activos):
                                    with cols_discos[idx]:
                                        renderizar_barra_estado(
                                            ...
                                            pct_libre=disco['pct'], gb_libres=disco['libres'], gb_totales=disco['total'],
    ...
    with tab2:
        ...
        serv_seleccionado_conf = st.selectbox("Selector de Servidores para Edición de Umbrales", options=opciones_servidores_p2, label_visibility="collapsed", key="sb_conf_umbrales")
        ...
        if serv_seleccionado_conf == VALOR_DEFECTO_P2:
            ...
            if serv_conf_info:
                ...
                st.markdown('<h3 style="font-size:17px; color:#003366; margin-top:25px; font-weight:bold;">🎛️ Configuración de Cores Lógicos (Globalizado)</h3>', unsafe_allow_html=True)
                # Corregido: Agrupados bajo la configuración de las 3 columnas reales 'cpu_p_'
                dict_nuevos_valores["cpu_p_buen_estado"] = st.slider("🟢 Estable Cores (% Uso Máx)", 1, 100, int(umbrales_vivos.get("cpu_p_buen_estado", 69)), key="p2_cores_ok")
                dict_nuevos_valores["cpu_p_advertencia"] = st.slider("⚠️ Advertencia Cores (% Uso)", 1, 100, int(umbrales_vivos.get("cpu_p_advertencia", 70)), key="p2_cores_adv")
                dict_nuevos_valores["cpu_p_critico"] = st.slider("🔴 Crítico Cores (% Uso)", 1, 100, int(umbrales_vivos.get("cpu_p_critico", 85)), key="p2_cores_crit")
                ...
                col_net_c1, col_net_c2, col_net_c3 = st.columns(3)
                with col_net_c1:
                    dict_nuevos_valores["red_limite_total_mbps"] = st.slider("Límite Total (Mbps)", 10, 1000, int(umbrales_vivos.get("red_limite_total_mbps", 100)), key="p2_red_t")
                with col_net_c2:
                    dict_nuevos_valores["red_limite_entrante_mbps"] = st.slider("Límite Entrante (Mbps)", 10, 1000, int(umbrales_vivos.get("red_limite_entrante_mbps", 50)), key="p2_red_e")
                with col_net_c3:
                    dict_nuevos_valores["red_limite_saliente_mbps"] = st.slider("Límite Saliente (Mbps)", 10, 1000, int(umbrales_vivos.get("red_limite_saliente_mbps", 50)), key="p2_red_s")
                ...
                with col_lat_2:
                    dict_nuevos_valores["perdida_limite_pct"] = st.slider("Límite Pérdida Paquetes (%)", 1, 20, int(umbrales_vivos.get("perdida_limite_pct", 1)), key="p2_perdida_limite")

                # CANALES DE ALMACENAMIENTO ESTÁTICO (Discos)
                ...
                if discos_configurables:
                    ...
                    for idx, d_conf in enumerate(discos_configurables):
                        ...
                        with cols_discos_conf[idx]:
                            ...
                            dict_nuevos_valores[f"disco_{d_num}_buen_estado"] = st.slider(f"🟢 Estable d{d_num}", 1, 100, int(umbrales_vivos.get(f"disco_{d_num}_buen_estado", 25)), label_visibility="collapsed", key=f"p2_ok_{d_num}")
                            dict_nuevos_valores[f"disco_{d_num}_advertencia"] = st.slider(f"⚠️ Adv d{d_num}", 1, 100, int(umbrales_vivos.get(f"disco_{d_num}_advertencia", 15)), label_visibility="collapsed", key=f"p2_adv_{d_num}")
                            dict_nuevos_valores[f"disco_{d_num}_critico"] = st.slider(f"🔴 Crit d{d_num}", 1, 100, int(umbrales_vivos.get(f"disco_{d_num}_critico", 5)), label_visibility="collapsed", key=f"p2_crit_{d_num}")
