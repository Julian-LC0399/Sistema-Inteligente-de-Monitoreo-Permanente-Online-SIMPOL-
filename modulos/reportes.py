...
def mostrar_pantalla():
    st.markdown(
        "<h2 style='color:#003366;'>📊 Reportes e inteligencia predictiva</h2>",
    ...
    with st.form("filtro_reportes"):
        ...
        btn_generar = st.form_submit_button(
            "Consultar Histórico", use_container_width=True
        )
    ...
    if btn_generar:
        try:
            ...
            query = f"""
                SELECT fecha_registro as Fecha, 'Nodo-CSU' as Nodo, uso_cpu as 'CPU %', uso_ram as 'RAM %' 
            ...
            if not df.empty:
                ...
                for _, row in df.head(
                    100
                ).iterrows():  # Limitamos a 100 para el PDF por rendimiento
