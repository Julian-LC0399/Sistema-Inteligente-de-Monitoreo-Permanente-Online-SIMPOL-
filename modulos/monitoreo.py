...
def mostrar_pantalla(user_actual):
    ...
    try:
        ...
        if not df_m.empty:
            ...
            st.info("💡 No se encontraron registros en la tabla 'monitoreo_nodos'. Verifique que el agente de base de datos esté activo.")
    ...
    with st.expander("Ver recomendaciones de optimización"):
        if cpu_val > 80:
            st.warning("Se detecta una carga alta de CPU. Considere revisar los procesos de PRTG o cierres de tareas pendientes.")
        else:
            st.success("El rendimiento del procesador se mantiene dentro de los parámetros estables.")