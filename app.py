...
if not st.session_state["autenticado"]:
    ...
    with placeholder.container():
        ...
        with col_login:
            st.markdown(
                "<h1 class='nombre-sistema'>SIMPOL</h1>", unsafe_allow_html=True
            )
            st.markdown(
                "<p class='subtitulo-sistema'>SISTEMA INTELIGENTE DE MONITOREO PERMANENTE ONLINE</p>",
            ...
            with st.form("form_acceso"):
                ...
                if st.form_submit_button("AUTENTICAR"):
                    ...
                    if user_info:
                        ...
                        st.session_state["nombre_analista"] = user_info[
                            "nombre_completo"
                        ]
...
# --- 4. DASHBOARD Y NAVEGACIÓN ---
    ...
    with st.sidebar:
        ...
        nombre_display = (
            st.session_state.get("nombre_analista") or st.session_state["user_actual"]
        )
    ...
    if seleccion == "🏠 Inicio":
        st.markdown(
            f"<h1 style='color:#003366;'>Bienvenido, {nombre_display}</h1>",
            unsafe_allow_html=True,
        )
        st.write("---")
        ...
        with col2:
            st.info(
                f"**Estatus de Conexión**\n\n**Analista:** {st.session_state['user_actual']}\n\n**Rol:** {rol_display}\n\n**Base de Datos:** Conectada"
            )

    elif seleccion == "📊 Monitoreo en Vivo":
        st.markdown(
            f"<h2 style='color:#003366; margin-top:-30px;'>Monitoreo en Tiempo Real: Nodo CSU</h2>",
            unsafe_allow_html=True,
        )
        ...
        try:
            ...
            df = pd.read_sql(
                "SELECT fecha_registro, uso_cpu, uso_ram FROM monitoreo_30_nodos ORDER BY id DESC LIMIT 40",
                conn,
            )
            conn.close()
        ...
        st.markdown(
            "<h2 style='color:#003366; margin-top:-30px;'>Generación de Reportes</h2>",
            unsafe_allow_html=True,
        )
        st.write("---")
        ...
        st.markdown(
            "<h2 style='color:#003366; margin-top:-30px;'>Gestión de Usuarios CSU</h2>",
            unsafe_allow_html=True,
        )
        st.write("---")
        ...
        with col_form:
            st.markdown(
                "<h4 style='color:#003366;'>📝 Nuevo Registro de Analista</h4>",
                unsafe_allow_html=True,
            )
            with st.form("reg_user", clear_on_submit=True):
                ...
                if st.form_submit_button(
                    "REGISTRAR EN SISTEMA", use_container_width=True
                ):
                    if n_user and n_name and n_pass:
                        try:
                            ...
                            cursor = conn.cursor()
                            cursor.execute(
                                "INSERT INTO usuarios (usuario, clave, nombre_completo, rol) VALUES (%s, %s, %s, %s)",
